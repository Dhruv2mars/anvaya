...
Anvaya (“the App”) is a local-first personal Panchang and daily ratings app.

## Data we store

All core data is stored in a local SQLite database in the App’s private storage on your device:

- Personal metric names you create
- Daily ratings (1–5) and optional one-line notes
- Cached Panchang fields for days you open
- Coordinates used for Panchang (see Location)
...
If you grant location permission, the App may use **precise** coordinates (when the platform provides them) to compute local sunrise and Hindu Panchang. Coordinates may be:

1. Cached in settings (`location_cache`) for reuse, and
2. Stored on each day record you open (`latitude` / `longitude` on that day’s row) so historical Panchang can be reproduced.
